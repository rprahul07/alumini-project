    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-1 sm:p-2 z-50">
      <div className="bg-white/95 backdrop-blur-xl rounded-xl lg:rounded-2xl w-full max-w-sm sm:max-w-md md:max-w-lg max-h-[90vh] overflow-y-auto scrollbar-hide p-3 sm:p-4 lg:p-6 border border-slate-200 shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-900 font-display">Applicant Profile</h2>
        ...
        <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
          <div className="relative h-24 sm:h-28 lg:h-36 bg-slate-100 rounded-xl lg:rounded-2xl mb-3 sm:mb-4 border border-slate-200">
          {data.photoUrl ? (
            <img 
              src={data.photoUrl} 
              alt={data.name} 
            ...
            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-100 to-secondary-100 rounded-xl lg:rounded-2xl">
              <AcademicCapIcon className="h-8 w-8 sm:h-10 sm:w-10 lg:h-12 lg:w-12 text-primary-600" />
            </div>
          )}
          ...
          <div className="absolute top-2 left-2">
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border ${
              data.role === 'alumni' ? 'bg-blue-100 text-blue-700 border-blue-200' :
              data.role === 'student' ? 'bg-green-100 text-green-700 border-green-200' :
          ...
          {/* Applicant Name */}
          <h3 className="font-bold text-slate-900 mb-2 line-clamp-2 text-xl sm:text-2xl leading-tight">
            {data.name}
          </h3>
        </div>
        ...
        <div>
          ...
          {/* Contact Information */}
          <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
            <h4 className="text-base font-semibold text-slate-900 mb-1">Contact Information</h4>
            <div className="space-y-1">
              {data.email && (
                <div className="flex items-center text-xs sm:text-sm text-slate-600">
                  <EnvelopeIcon className="h-4 w-4 mr-2 text-primary-600" />
                  <a 
              ...
              {data.phone && (
                <div className="flex items-center text-xs sm:text-sm text-slate-600">
                  <PhoneIcon className="h-4 w-4 mr-2 text-primary-600" />
                  <a 
                    href={`tel:${data.phone}`}
                    className="text-primary-600 hover:text-primary-700 transition-colors"
                  >
              ...
              {!data.email && !data.phone && (
                <div className="text-xs text-slate-500 italic">No contact information available</div>
              )}
            </div>
          </div>
          ...
          {(data.linkedInProfile || data.githubProfile || data.twitterProfile) && (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Social Links</h4>
          ...
          {(data.highestQualification || data.passoutYear || data.course || data.department || data.currentSemester || data.rollNumber || data.batchStartYear || data.batchEndYear) && (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Education & Academic Details</h4>
              <div className="space-y-1">
                {data.department && (
                  <div className="flex items-center text-xs sm:text-sm text-slate-600">
                ...
                {data.highestQualification && (
                  <div className="flex items-center text-xs sm:text-sm text-slate-600">
                    ...
                    {data.passoutYear && (
                      <span className="text-slate-500 ml-1">({data.passoutYear})</span>
                    )}
                  </div>
                )}
                {data.course && (
                  <div className="flex items-center text-xs sm:text-sm text-slate-600 ml-6">
                ...
                {data.rollNumber && (
                  <div className="flex items-center text-xs sm:text-sm text-slate-600">
                    <IdentificationIcon className="h-4 w-4 mr-2 text-primary-600" />
          ...
          {(data.currentJobTitle || data.companyName || data.companyRole || data.totalExperience) && (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Experience</h4>
              <div className="space-y-1">
                {data.currentJobTitle && (
                  <div className="flex items-center text-xs sm:text-sm text-slate-600">
                    ...
                    {data.companyName && (
                      <span className="text-slate-500 ml-1">at {data.companyName}</span>
                    )}
                  </div>
                )}
                {data.companyRole && (
          ...
          {data.skills && data.skills.length > 0 ? (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Skills</h4>
          ...
          {data.bio ? (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Bio</h4>
              <div className="bg-slate-100 rounded-lg p-3 text-slate-600 text-sm leading-relaxed whitespace-pre-line break-words">
                {data.bio}
              </div>
            </div>
          ) : (
            <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
              <h4 className="text-base font-semibold text-slate-900 mb-1">Bio</h4>
              <div className="text-xs text-slate-500 italic">No bio available</div>
            </div>
          )}

          {/* Resume/CV */}
          <div className="bg-white/80 backdrop-blur-xl rounded-xl lg:rounded-2xl p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6 border border-slate-200">
            <h4 className="text-base font-semibold text-slate-900 mb-1">Resume / CV</h4>
