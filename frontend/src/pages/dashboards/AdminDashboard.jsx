...
const EventManagementSection = ({ eventSection }) => {
  ...
  const fetchEvents = async (page = 1) => {
    ...
    setError(null);
    try {
      const response = await axios.get(`/api/admin/event/all?page=${page}&limit=${limit}`);
      const { events: fetchedEvents, pagination } = response.data.data;
      
      // Filter events by role and status
      let filteredEvents = fetchedEvents.filter(event => {
        const roleMatch = eventSection === 'all' || event.user?.role === eventSection;
  ...
  }, [eventSection, activeTab, searchTerm, currentPage]);
  ...
  const handleStatusChange = async (eventId, newStatus) => {
    ...
    try {
      ...
      await axios.patch(`/api/admin/event/${eventId}`, updatedEvent);
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      await axios.delete(`/api/admin/event/${deleteEventId}`);
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        {['pending', 'approved', 'rejected', 'all'].map((status) => (
          ...
          >
            {status.charAt(0).toUpperCase() + status.slice(1)} ({filteredEvents.filter(e => status === 'all' || e.status === status).length})
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-gray-500">
          No {activeTab === 'all' ? '' : activeTab} events found for {eventSection === 'all' ? 'any role' : eventSection} users.
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((event) => (
            <div key={event.id} className="bg-gray-50 rounded-xl p-6 border border-gray-200 hover:shadow-md transition-shadow">
              {/* Event Image */}
              {event.imageUrl && (
                <div className="mb-4">
                  <img 
              ...
              <div className="mb-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="text-lg font-semibold text-gray-900 truncate" title={event.name}>
                    {event.name}
                  </h3>
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <div className="flex items-center">
                    ...
                    <span>Creator: {event.user?.fullName || 'Unknown'} ({event.user?.role || 'Unknown'})</span>
                  </div>
                  <div className="flex items-center">
                  ...
                  {event.maxCapacity && (
                    <div className="flex items-center">
                      ...
                      <span>Max: {event.maxCapacity} people</span>
                    </div>
                  )}
                </div>
              </div>
...
const EventDetailsModal = ({ event, isOpen, onClose, onStatusChange, onDelete }) => {
      ...
      <div className="bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              <h4 className="text-lg font-semibold text-gray-900 mb-4">{event.name}</h4>
              <div className="space-y-3 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 text-gray-600">
                    {event.user?.fullName || 'Unknown'} ({event.user?.role || 'Unknown'})
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-3 text-sm">
                ...
                {event.maxCapacity && (
                  <div>
                    ...
                    <span className="ml-2 text-gray-600">{event.maxCapacity} people</span>
                  </div>
                )}
                <div>
